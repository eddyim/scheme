...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        first, second = formals, vals
        while first != nil and second != nil:
            child.define(first.first, second.first)
            first, second = first.second, second.second
        if not (first == nil and second == nil):
            raise SchemeError('unexpected amount of formal parameters')
        # END PROBLEM 11
        return child
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('unexpected amount of arguments passed')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        function_expression = Pair(target.second, expressions.second)
        env.define(function_name, do_lambda_form(function_expression, env))
...
def do_and_form(expressions, env):
    ...
    while exp.second != nil:
        if scheme_falsep(scheme_eval(exp.first, env)):
...
def do_or_form(expressions, env):
    ...
    if exp == nil:
        return False
    while exp.second != nil:
        if scheme_truep(scheme_eval(exp.first, env)):
            return scheme_eval(exp.first, env)
        exp = exp.second
    return scheme_eval(exp.first, env, True)
    # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    formals, values = Pair(current_binding.first, nil), Pair(scheme_eval(current_binding.second.first, env), nil)
    f_pointer, v_pointer = formals, values
    while bindings.second != nil:
        bindings = bindings.second
        current_binding = bindings.first
        check_form(current_binding, 2, 2)
        f_pointer.second, v_pointer.second = Pair(current_binding.first, nil), Pair(scheme_eval(current_binding.second.first, env), nil)
        f_pointer, v_pointer = f_pointer.second, v_pointer.second
    check_formals(formals)
